...
struct LocalPlanAssistant: PlanAssisting {
    func extract(_ message: String, profiles: [UserProfile], now: Date = Date()) -> ParsedPlan {
        ...
        if let profile = profiles.first(where: { lower.range(of: "\\b" + NSRegularExpression.escapedPattern(for: $0.name.lowercased()) + "\\b", options: .regularExpression) != nil }) {
        ...
        if let activity = activities.first(where: { lower.range(of: "\\b" + $0.0 + "\\b", options: .regularExpression) != nil }) {
            draft.activityName = activity.1
            draft.activityType = activity.2
            captured.activity = true
        ...
        if hasDate { captured.date = true }
        else { missing.append("Confirm the date.") }
        draft.startTime = day
        ...
        if lower.contains("hour and a half") { draft.durationMinutes = 90; captured.duration = true }
            ...
            captured.duration = true
        } else if lower.contains("for an hour") || lower.contains("for one hour") { draft.durationMinutes = 60; captured.duration = true }
        ...
        return .init(draft: draft, missing: missing, captured: captured)
    }
}
