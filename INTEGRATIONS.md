...
- **Apple Health (permission only):** `HealthKitService` requests read access to heart rate and respiratory rate via a Connect button in Settings, backed by the `com.apple.developer.healthkit` entitlement and an `NSHealthShareUsageDescription` string. No HealthKit data is read, stored, or used anywhere yet; this only clears the permission for a future correlation feature.
